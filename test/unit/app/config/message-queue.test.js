...
describe('Message queue Config Test', () => {
  ...
  test('Invalid env var throws error', () => {
    try {
      ...
      expect(err.message).toBe('The message queue config is invalid. "applicationRequestQueue.host" must be a string. "applicationResponseQueue.host" must be a string. "paymentRequestTopic.host" must be a string. "paymentResponseSubscription.host" must be a string')
    }
  })
})
