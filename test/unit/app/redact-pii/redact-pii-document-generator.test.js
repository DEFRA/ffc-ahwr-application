...
import { redactPII } from '../../../../app/redact-pii/redact-pii-document-generator'
...
describe('redact-pii-document-generator', () => {
  const endpoint = `${config.documentGeneratorApiUri}/redact/pii`
  ...
  beforeEach(() => {
    agreementsToRedact = [
      { reference: 'AHWR-123', data: { sbi: 'SBI001' }, redactedSbi: '752134284' },
      { reference: 'AHWR-456', data: { sbi: 'SBI002' }, redactedSbi: '558234924' }
    ]
    logger = { setBindings: jest.fn() }
    jest.clearAllMocks()
  })

  it('should successfully call document generator redact pii api with the correct payload', async () => {
    ...
    expect(wreck.post).toHaveBeenCalledWith(endpoint, {
      ...
      payload: {
        agreementsToRedact: [
          { reference: 'AHWR-123', sbi: 'SBI001', redactedSbi: '752134284' },
          { reference: 'AHWR-456', sbi: 'SBI002', redactedSbi: '558234924' }
        ]
      }
    })
