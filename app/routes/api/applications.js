...
import { getHerdsByAppRefAndLivestock } from '../../repositories/herd-repository.js'
  ...
  },
  {
    ...
    options: {
      validate: {
        ...
        query: joi.object({
          typeOfLivestock: joi.string().valid(...Object.values(livestockTypes)),
        }),
        failAction: async (request, h, err) => {
          request.logger.setBindings({ err })
          return h.response({ err }).code(HttpStatus.BAD_REQUEST).takeover()
      ...
      handler: async (request, h) => {
        ...
        request.logger.setBindings({ ref, typeOfLivestock })

        const herds = await getHerdsByAppRefAndLivestock(ref, typeOfLivestock)
